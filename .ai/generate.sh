...
declare -A IMAGES
for f in "$PROJECT_ROOT"/infra-*/docker-compose.yml; do
    [ ! -f "$f" ] && continue
...
declare -A ENV_VARS
for f in "$PROJECT_ROOT"/infra-*/.env.sandbox; do
    [ ! -f "$f" ] && continue
...
COMPACT_RULES="PROJECT: Infra-IAM-PKI (internal PKI + Keycloak SSO + Open OnDemand portal)
